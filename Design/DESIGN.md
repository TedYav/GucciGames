...
3. Game Player
	The Game Player is the front-end component of the environment the users uses to play the game. It interacts with both the Game Data and the Game Engine.To start, the user can choose to load a game from Game Data. During the game play, the Game Engine acts as the backend of Game Player by carrying out the game logic. During any stage of the game, the user can choose to save the current game state directly into the game data. 
