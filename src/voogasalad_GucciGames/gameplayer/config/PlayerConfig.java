...
public class PlayerConfig {
	...
	/**
	 * Returns the specified parameter in ResourceBundle bundle. i.e. if I want
	 * PlayerColor15 I would call getResourceNumber(bundle, "PlayerColor", 15);
	 * 
	 * @param bundle
	 * @param prefix
	 * @param number
	...
	public static String getResourceNumber(ResourceBundle bundle, String prefix, Integer number) {
		return bundle.keySet().stream().filter(s -> s.startsWith(prefix)).filter(s -> s.endsWith(number.toString()))
				.map(s -> bundle.getString(s)).collect(Collectors.toList()).get(0);
	}
