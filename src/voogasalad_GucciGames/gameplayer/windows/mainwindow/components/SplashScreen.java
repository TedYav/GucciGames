...
public class SplashScreen extends WindowComponent {
	...
	private double getValue(String key) {
		return myConfig.containsKey(key) ? Double.parseDouble(myConfig.getString(key))
				: Double.parseDouble(myMainConfig.getString(key));
	}
	...
	private void initializeText(String text) {
		...
		myText = new Text();
		myText.setText(text);
		myText.getStyleClass().addAll(myConfig.keySet().stream().filter((s) -> s.contains("Class"))
				.map(s -> myConfig.getString(s)).collect(Collectors.toList()));
	...
	private void setAdvanceTimer() {
		if (myDuration > 0) {
			Timeline timeline = new Timeline(
					new KeyFrame(Duration.millis(myDuration), e -> getGameScene().getManager().sceneFinished()));
			timeline.play();
		}
