	...
	public NetworkConfigScene(GameSceneManager manager, GameWindow window, String config) {
		...
		ipDialog.setOnAction(e -> {
			...
			enter.setOnAction(eh -> {
				...
				try {
					getManager().getController().getEngine()
							.setController((GameControllerEngineInterface) getManager().getController());
			...
			dialogVbox.setOnKeyPressed(key -> {
				if (key.getCode().equals(KeyCode.ENTER)) {
					enter.fire();
				}
			});
			Scene dialogScene = new Scene(dialogVbox, Integer.parseInt(myConfig.getString("DialogWidth")),
					Integer.parseInt(myConfig.getString("DialogHeight")));
