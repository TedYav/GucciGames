...
public class SelectLevelScene extends GameMenuScene {
	...
	protected Map<String, MenuAction> buildOptionMap() {
		...
		levels.keySet().stream().forEach(s -> System.out.println(levels.get(s).getLevelName()));
		levels.keySet().stream().map(name -> levels.get(name)).filter(level -> level.isMyChoosability())
