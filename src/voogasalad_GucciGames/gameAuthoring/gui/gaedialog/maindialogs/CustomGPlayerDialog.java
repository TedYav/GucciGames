...
public class CustomGPlayerDialog<T> extends AGaeDialog<T> {
	...
	private ResourceBundle namesBundle = ResourceBundle
	...
	private int numBars;
	private int maxBars = 3; // assume there are only 3 bars: left, bottom,
								// right
	...
	public CustomGPlayerDialog(IDialogGaeController dialogGaeController, IGuiGaeController guiGaeController) {
		...
		prop = helper.loadProperties(
				"/voogasalad_GucciGames/gameAuthoring/gui/gaedialog/maindialogs/dialogproperties/customgplayerdialog.properties");
				...
				for (List<CheckBoxField> cb : checkBoxFields) {
					for (CheckBoxField cbf : cb) {
						if (cbf.getCheckBox().isSelected()) {
							if (!allCheckedBoxes.get(Integer.parseInt((String) cbf.getUserData()))
									.contains(cbf.getPropKey())) {
			...
			for (numBars = 0; numBars < maxBars; numBars++) {
				prop.forEach(((key, value) -> {
					...
					if (svalue.equals(guiList.get(propIndexer))) {
						actionDisplay = new CheckBoxField(dialogElements,
								(skey.split(prop.getProperty("componentdelimiter")))[1]);
					}
				}));
	...
	// currCheckBoxField.getCheckBox().selectedProperty().addListener((new
	// ChangeListener<Boolean>() {
	// public void changed(ObservableValue<? extends Boolean> ov,
	// Boolean old_val, Boolean new_val) {
	// if(new_val){
	// allCheckedBoxes.get(Integer.parseInt((String)
	// currCheckBoxField.getUserData())).add(currCheckBoxField.getPropKey());
	// }else{
	// allCheckedBoxes.get(Integer.parseInt((String)
	// currCheckBoxField.getUserData())).remove(currCheckBoxField.getPropKey());
	// }
	// }
	// }));
	// }
