...
public class XStreamGameEngine {
	...
	public void saveGameInfo(GameInfo game, File file) {
		try {
			...
			myManager.addGame(game.getGameName(),
					sanitizeGameName(game.getGameName()) + myConfig.getString("GameExtension"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void saveGameState(GamePlayerSave game, File file) {
		try {
			...
			myLoader.save(file, gameXML);
			// myManager.addGame(game.getGameName(),
			// sanitizeGameName(game.getGameName())+
			// myConfig.getString("GameExtension"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void saveGameState(GamePlayerSave game) {
		saveGameState(game,
				new File(gameNameToPathName(game.getInfo().getGameName()) + myConfig.getString("SaveDirectory")
						+ DateFormat.getDateInstance().format(new Date()) + " "
						+ DateFormat.getTimeInstance().format(new Date()).replace(':', '-')
						+ myConfig.getString("SaveExtension")));
	}
	...
	private StringBuilder sanitizeGameName(String name) {
		...
		sanitizedName.append(myConfig.getString("GameStorageLocation"))
				.append(name.replaceAll("[^a-zA-Z0-9\\._]+", "_"));
